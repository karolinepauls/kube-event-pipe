...
def test_run_event_pipe(kube_api, clean_kube, kube_event_pipe: KubeEventPipe) -> None:
    ...
    with kube_event_pipe(output_file_name='first.log') as event_pipe:
        assert event_pipe.get_events() == [], 'Expecting clean state'
        for _ in range(3):
            make_kube_event(kube_api)

        events = event_pipe.get_events(min_count=3)
        assert len(events) == 3

    with kube_event_pipe(output_file_name='second.log') as event_pipe:
        no_events = event_pipe.get_events()
        assert no_events == [], 'The program, if restarted, should skip events already seen.'

    with kube_event_pipe(output_file_name='third.log', state_subdir='another') as event_pipe:
        same_events = event_pipe.get_events()
...
def test_event_identity(kube_api, clean_kube, kube_event_pipe: KubeEventPipe) -> None:
    ...
    with kube_event_pipe(output_file_name='first.log') as event_pipe:
        ...
        with pytest.raises(Timeout):
            event_pipe.get_events(min_count=1, timeout=1)
